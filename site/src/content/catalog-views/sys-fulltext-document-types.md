...
## Returns a row for each document type that is available for full-text indexing operations. Each
row represents the IFilter interface that is registered in the instance of SQL Server.
...
full-text indexing of columns of type
...
The path to the IFilter DLL. The path is only visible to members of
...
Note: Only documents with the manufacturer as Microsoft are

supported on SQL Database.
