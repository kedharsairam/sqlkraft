...
Article • 02/28/2023 Applies to: SQL Server Translates the permissions bitmask returned by SQL Trace into a table of permissions names. Transact-SQL syntax conventions level Is the kind of securable to which the permission is applied. level is . perms Is a bitmask that is returned in the permissions column. perms is . The value returned in the column of a SQL Trace is an integer representation of a bitmask used by SQL Server to calculate effective permissions. Each of the 25 kinds of securables has its own set of permissions with corresponding numerical values. translates this bitmask into a table of permissions names. Requires membership in the role.
