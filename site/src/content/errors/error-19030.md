---
name: '19030'
title: 'Error 19030'
errorNumber: 19030
severity: 'medium'
category: 'io'
description: 'Description 19030 10 Yes SQL Trace ID %d was started by login "%s". 19031 10 Yes SQL Trace stopped. Trace ID = ''%d''. Login Name = ''%s''. 19032 10 Yes SQL Trace was stopped due to server shutdown. Trace'
tags: ["io", "severity-medium"]
pubDate: 2026-05-29
---

## Description

Description 19030 10 Yes SQL Trace ID %d was started by login "%s". 19031 10 Yes SQL Trace stopped. Trace ID = '%d'. Login Name = '%s'. 19032 10 Yes SQL Trace was stopped due to server shutdown. Trace

## Severity

MEDIUM (Level N/A)

## Troubleshooting

*(Diagnostic content pending full extraction)*

