---
name: '7940'
title: 'Error 7940'
errorNumber: 7940
severity: 'medium'
category: 'io'
description: 'Description 7940 16 No System databases master, model, msdb, and tempdb cannot be detached. 7941 16 No Table error: object ID %d, index ID %d, partition ID %I64d processing encountered file name "%.*l'
tags: ["io", "severity-medium"]
pubDate: 2026-05-29
---

## Description

Description 7940 16 No System databases master, model, msdb, and tempdb cannot be detached. 7941 16 No Table error: object ID %d, index ID %d, partition ID %I64d processing encountered file name "%.*l

## Severity

MEDIUM (Level N/A)

## Troubleshooting

*(Diagnostic content pending full extraction)*

