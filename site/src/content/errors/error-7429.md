---
name: '7429'
title: 'Error 7429'
errorNumber: 7429
severity: 'medium'
category: 'io'
description: 'Description interface, but returned "%ls" for "%ls". 7429 10 Yes %hs SQL Server Remote Metadata Gather Time for Table %s.%s:%hs, CPU time = %lu ms, elapsed time = %lu ms. 7430 16 Yes Out-of-process us'
tags: ["io", "severity-medium"]
pubDate: 2026-05-29
---

## Description

Description interface, but returned "%ls" for "%ls". 7429 10 Yes %hs SQL Server Remote Metadata Gather Time for Table %s.%s:%hs, CPU time = %lu ms, elapsed time = %lu ms. 7430 16 Yes Out-of-process us

## Severity

MEDIUM (Level N/A)

## Troubleshooting

*(Diagnostic content pending full extraction)*

