...
Occurs when a task voluntarily sleeps while waiting for an event such as WAITFOR, query timeout, or connection sleep. High counts are normal — this is the idle wait type.
...
Use the following query to identify the top queries contributing to `SLEEP_TASK` waits:
