...
Occurs when waiting for a shared latch on an in-memory structure. Normal at low levels; high values indicate contention on frequently accessed internal data structures.
...
Use the following query to identify the top queries contributing to `LATCH_SH` waits:
