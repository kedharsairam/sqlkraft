...
Occurs when a task waits for a data page to be written from the buffer pool to disk (exclusive latch). High durations indicate write latency on the storage subsystem.
...
Use the following query to identify the top queries contributing to `PAGEIOLATCH_EX` waits:
