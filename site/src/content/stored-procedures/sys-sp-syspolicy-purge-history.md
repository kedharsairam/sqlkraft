...
06/23/2025 Applies to: SQL Server Removes the policy evaluation history according to the history retention interval setting. Transact-SQL syntax conventions syntaxsql This stored procedure has no parameters. (success) or (failure). You must run in the context of the system database. To view the history retention interval, you can use the following query: SQL If the history retention interval is set to , policy evaluation history isn't removed.
