...
This stored procedure is used to update identity range assignments. It ensures that automatic identity range management functions properly after a Publisher is restored from a backup. This stored procedure is executed at the Publisher on the publication database. Transact-SQL syntax conventions The name of the publication. , with a default of . When specified, only identity ranges for that publication are restored. The name of the article. , with a default of . When specified, only identity ranges for that article are restored.
