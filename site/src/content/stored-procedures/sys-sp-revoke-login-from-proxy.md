...
category: 'general'
description: 'Removes access to a proxy for a security principal. Transact-SQL syntax conventions The name of the SQL Server login, server role, or database role for which to remove The ID of the proxy for which to remove access. must be specified, but both can''t be specified. The name of the proxy for which to remove access. must be specified, but both can''t be specified.'
