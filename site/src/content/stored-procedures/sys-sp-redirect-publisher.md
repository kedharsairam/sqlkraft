...
Specifies a redirected publisher for an existing publisher/database pair. If the publisher database belongs to an Always On availability group (AG), the redirected publisher is the AG listener name associated with the AG. Transact-SQL syntax conventions The name of the instance of SQL Server that originally published the database. The name of the database being published. The AG listener name associated with the AG that will be the new publisher.
