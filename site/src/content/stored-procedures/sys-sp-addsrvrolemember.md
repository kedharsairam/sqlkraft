...
category: 'general'
description: 'Adds a login, or security principal, as a member of a fixed server role. Transact-SQL syntax conventions The name of the security principal being added to the fixed server role. can be a SQL Server login or a Windows account. If the Windows account isn''t already granted access to SQL Server, access is automatically granted. The name of the fixed server role to which the security principal is being'
