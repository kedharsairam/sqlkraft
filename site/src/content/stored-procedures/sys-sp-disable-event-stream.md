---
name: "sys.sp_disable_event_stream"
title: "sys.sp_disable_event_stream"
category: "general"
description: "Disables change event streaming at the database level for the current database context, including all associated tables. What is change event streaming (preview)? SQL Server 2025 (17.x) and Azure SQL Database. Transact-SQL syntax conventions server role membership can execute this procedure. What is change event streaming (preview)? Configure change event streaming (preview) Change event streaming"
tags: ["stored-procedure"]
pubDate: 2026-05-29
syntax: |
  sys.sp_disable_event_stream
  [ ; ]
---

## Description

Disables change event streaming at the database level for the current database context, including all associated tables. What is change event streaming (preview)? SQL Server 2025 (17.x) and Azure SQL Database. Transact-SQL syntax conventions server role membership can execute this procedure. What is change event streaming (preview)? Configure change event streaming (preview) Change event streaming is currently in

## Syntax

```sql
sys.sp_disable_event_stream
[ ; ]
```
